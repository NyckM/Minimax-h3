...
def _pixels_for_tokens(n_tokens):
    """Quantidade de frames visíveis produzida pelos primeiros tokens H3."""
    return sum(_FRAME_PER_TOKEN[i % 5] for i in range(max(0, int(n_tokens))))


def compute_seam_boundaries(seg_sizes, effective_context):
    """Calcula as emendas exatas no vídeo decodificado a partir dos tokens.

    Usa a mesma contabilidade do merge latent, evitando estimar a posição das
    emendas pela duração nominal dos chunks.
    """
    if not seg_sizes:
        return [], 0
    if len(seg_sizes) < 2:
        return [], _pixels_for_tokens(video_latent_frames(seg_sizes[0]))
    segment_tokens = [video_latent_frames(size) for size in seg_sizes]
    context_tokens = video_latent_frames(effective_context) if effective_context > 5 else 2
    context_tokens = max(2, context_tokens)
    boundaries = []
    accumulated = segment_tokens[0]
    for index in range(1, len(segment_tokens)):
        boundaries.append(_pixels_for_tokens(accumulated))
        removed = min(context_tokens, segment_tokens[index] - 2)
        accumulated += segment_tokens[index] - removed
    return boundaries, _pixels_for_tokens(accumulated)
...
def run_auto_context_generation(model, vae, audio_vae, clip,
    ...
    requested_continuation = str(
        continuation_method or native_masked.CONTINUATION_GUIDE)
    resolved_continuation, native_issues = native_masked.resolve_method(
        requested_continuation)
    driving_audio_requested = bool(audio_drive and drive_audio is not None)
    if resolved_continuation == native_masked.CONTINUATION_NATIVE:
        original_context = int(context_frames)
        context_frames = native_masked.normalize_context_frames(
            context_frames,
            fps=fps,
            carry_generated_audio=not driving_audio_requested,
        )
        if int(context_frames) != original_context:
            print(
                f"[H3-Auto] Native Masked AV: context_frames {original_context} -> "
                f"{context_frames} (39 frames = 65 tokens de áudio a 24 fps)"
            )
    elif requested_continuation == native_masked.CONTINUATION_AUTO and native_issues:
        print(
            "[H3-Auto] Auto: Core sem máscara H3 por token; usando Guide / "
            "Motion Context. Para Native Masked atualize o ComfyUI."
        )
    print(
        f"[H3-Auto] Continuação: solicitado='{requested_continuation}' "
        f"resolvido='{resolved_continuation}'"
    )
    ...
    if (
        resolved_continuation == native_masked.CONTINUATION_NATIVE
        and not driving_audio_requested
        and int(fps) == 24
        and effective_context != 39
    ):
        raise native_masked.NativeMaskedContinuationError(
            "Native Masked com áudio gerado precisa de 39 frames de contexto, "
            "mas o tamanho do chunk permitiu apenas "
            f"{effective_context}. Aumente chunk_frames para pelo menos 56."
        )
    ...
    for idx, (start_f, end_f) in enumerate(chunks):
        ...
        if (
            not is_first_chunk
            and resolved_continuation == native_masked.CONTINUATION_GUIDE
        ):
            anchor_prev = prev_x0
        ...
        if is_second_pass:
            ...
            if (
                ...
                overlap_tokens = (
                    max(2, video_latent_frames(effective_context))
                    if effective_context > 5 else 2
                )
                seg_latent_dict = _copy_overlap_tail(
                    seg_latent_dict, all_segments[idx - 1], overlap_tokens
                )
            if overlap_tokens > 0 or lock_audio:
                seg_latent_dict = _with_locked_audio(
                    seg_latent_dict,
                    overlap_video_tokens=overlap_tokens,
                    lock_audio=lock_audio,
                    overlap_video_denoise=video_context_denoise,
                )
        else:
            overlap_video = None
            if (
                ...
                overlap_video, _previous_audio = h3_conditioning.unpack_nested_latent(
                    all_segments[idx - 1]
                )
        ...
        if (
            not is_first_chunk
            and resolved_continuation == native_masked.CONTINUATION_NATIVE
        ):
            if prev_accepted is None:
                raise native_masked.NativeMaskedContinuationError(
                    "o chunk anterior aceito não está disponível para continuação"
                )
            carry_generated_audio = drive_waveform is None and not (
                is_second_pass and lock_audio
            )
            if drive_waveform is None and driving_audio_requested:
                raise native_masked.NativeMaskedContinuationError(
                    "o Driving Audio conectado não produziu waveform válido. "
                    "Corrija o áudio ou use 39 frames para continuidade AV gerada."
                )
            seg_latent_dict = native_masked.apply_native_prefix(
                seg_latent_dict,
                prev_accepted,
                context_frames=effective_context,
                fps=fps,
                carry_generated_audio=carry_generated_audio,
            )
            v_steps = video_latent_frames(effective_context)
            a_steps = (
                native_masked.exact_audio_prefix_steps(effective_context, fps)
                if carry_generated_audio else 0
            )
            print(
                f"[H3-Auto] Native Masked: prefixo protegido "
                f"video={v_steps} tokens, audio={a_steps} tokens"
            )
    ...
    seam_boundaries, seam_decoded_frames = compute_seam_boundaries(
        seg_sizes, effective_context
    )
    seam_info = {
        "boundaries": seam_boundaries,
        "seg_sizes": [int(size) for size in seg_sizes],
        "effective_context": int(effective_context),
        "decoded_frames": int(seam_decoded_frames),
    }
    if seam_boundaries:
        print(
            f"[H3 Auto Chunks/Bruxos] Emendas exatas após decode: "
            f"{seam_boundaries}; total previsto={seam_decoded_frames} frames."
        )
    ...
    if not decode_output:
        print("[H3 Auto Chunks/Bruxos] Decode interno desligado; retornando somente latents.")
...
def _make_h3_empty_latent(latent_w, latent_h, pixel_frames, fps,
                          drive_audio_latent=None, overlap_video=None,
                          overlap_video_denoise=0.0, overlap_frames=0):
    """Cria o latent AV e, no modo Guide, protege o começo já gerado."""
    ...
    if overlap_video is not None and overlap_frames > 0:
        context_tokens = video_latent_frames(overlap_frames) if overlap_frames > 5 else 2
        context_tokens = max(2, context_tokens)
        overlap_tokens = max(0, min(context_tokens, v_t - 2))
        if overlap_tokens > 0:
            video_latent[:, :, :overlap_tokens] = overlap_video[:, :, -overlap_tokens:].to(
                device=video_latent.device, dtype=video_latent.dtype
            )
            print(
                f"[H3 Auto Chunks/Bruxos] Continuação Guide: {overlap_tokens} tokens "
                f"copiados; força de redesenho={float(overlap_video_denoise):.2f}."
            )
    ...
    if locked or overlap_tokens > 0:
        ...
        if overlap_tokens > 0:
            video_mask[:, :, :overlap_tokens] = float(overlap_video_denoise)
        try:
...
def _copy_overlap_tail(latent_dict, previous_latent_dict, overlap_tokens):
    """Copia o final já refinado do chunk anterior para o começo do atual."""
    current_video, current_audio = h3_conditioning.unpack_nested_latent(latent_dict)
    previous_video, _previous_audio = h3_conditioning.unpack_nested_latent(previous_latent_dict)
    if current_video is None or previous_video is None:
        return latent_dict
    count = min(
        int(overlap_tokens), int(current_video.shape[2]) - 2,
        int(previous_video.shape[2]),
    )
    if count <= 0:
        return latent_dict
    current_video = current_video.clone()
    current_video[:, :, :count] = previous_video[:, :, -count:].to(
        device=current_video.device, dtype=current_video.dtype
    )
    if current_audio is not None:
        try:
            combined = comfy.nested_tensor.NestedTensor((current_video, current_audio))
        except Exception:
            combined = (current_video, current_audio)
    else:
        combined = current_video
    result = dict(latent_dict)
    result["samples"] = combined
    return result
...
def _with_locked_audio(latent_dict, overlap_video_tokens=0,
    ...
    """Cria a máscara do segundo passe para vídeo de contexto e áudio."""
    ...
    for t in tensors:
        if t.dim() == 5:
            ...
            count = min(int(overlap_video_tokens), int(t.shape[2]) - 2)
            if count > 0:
                mask[:, :, :count] = float(overlap_video_denoise)
                print(
                    f"[H3 Auto Chunks/Bruxos] Segundo passe: {count} tokens de "
                    f"contexto com redesenho={float(overlap_video_denoise):.2f}."
                )
            masks.append(mask)
        else:
