...
def compute_chunks(total_frames: int, chunk_frames: int, context_frames: int = 0):
    ...
    for s in sizes:
        ...
        # O início seguinte volta pela região de contexto. Esses pares também
        # definem a fatia temporal do source video; avançar por ``s`` fazia o
        # último chunk de vídeos longos apontar além do final da referência.
        start += s - context
...
def compute_tag_chunks(seg_target_frames_list, context_frames):
    ...
    for s in seg_sizes:
        chunks.append((start, start + s))
        # Os chunks compartilham context_frames; a coordenada nominal precisa
        # refletir essa sobreposição para prompt e referência segmentada.
        start += s - context_frames
