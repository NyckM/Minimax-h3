...
app.registerExtension({
    name: "Bruxos.H3UnlimitedPreview",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData?.name !== "BruxosH3UnlimitedPreview") return;
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            status.textContent = "Aguardando o H3 Sampler Unlimited…";
            ...
            node._minimaxUnlimitedPreview = data => {
                if (data.action === "reset") {
                    ...
                    imageUpdate++;
                    image.removeAttribute("src");
                    status.textContent = `Preparando ${chunkCount} chunks…`;
                    return;
                }
                if (data.execution !== execution) return;
                if (data.action === "complete") {
                    status.textContent = `Concluído · ${chunkCount} chunks`;
                ...
                status.textContent = `Chunk ${index + 1}/${data.chunk_count} · passo ${data.step}/${data.steps} · frames de saída ${data.output_start}-${data.output_end}`;
