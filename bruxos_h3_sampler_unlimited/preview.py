...
def _build_tiny_decoder(state_dict):
    ...
    for key, value in state_dict.items():
        ...
        if not separator or not index.isdigit():
            raise ValueError(f"chave não suportada no tiny VAE: {key}")
    ...
    for index in range(max(entries) + 1):
        ...
        if values is None:
            ...
            raise ValueError(f"camada não suportada no tiny VAE: {index}")
...
class _TinyDecoder:
    def __init__(self, name):
        ...
        if path is None:
            raise ValueError(f"tiny VAE '{name}' não encontrado em models/vae_approx")
...
def _tiny_frames(video, decoder, indices, max_resolution):
    if decoder.latent_channels != video.shape[1]:
        raise ValueError(f"o tiny VAE espera {decoder.latent_channels} canais latentes, mas o MiniMax H3 usa {video.shape[1]}")
...
class MiniMaxH3UnlimitedPreview(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="BruxosH3UnlimitedPreview",
            display_name="H3 Preview Acumulado Unlimited (Bruxos TESTE)",
            category="Bruxos do VFX/MiniMax H3/Chunks",
            description="Exibe em tempo real um preview acumulado dos chunks produzidos pelo H3 Sampler Unlimited (Bruxos TESTE).",
            inputs=[
                io.Model.Input("model"),
                io.Int.Input("max_resolution", default=512, min=64, max=2048, step=64,
                             tooltip="Maior lado do preview. Valores altos deixam o preview mais nítido, mas consomem mais CPU e memória."),
                io.Int.Input("quality", default=75, min=30, max=100, step=1,
                             tooltip="Qualidade WebP do preview, de 30 a 100."),
                io.Float.Input("fps", default=24.0, min=1.0, max=60.0, step=0.001,
                               tooltip="FPS usado somente para a duração correta do preview."),
                io.Int.Input("frame_stride", default=1, min=1, max=16, step=1,
                             tooltip="Mostra um a cada N frames latentes, preservando a duração de reprodução. Aumente para aliviar CPU."),
                io.Combo.Input("tiny_vae", options=["none"] + folder_paths.get_filename_list("vae_approx"), default="none",
                               tooltip="Decoder opcional de 24 canais, como taeh3.safetensors. none usa o preview Latent2RGB, mais simples e menos fiel."),
            ],
