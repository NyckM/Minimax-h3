...
try:
    ...
    SceneManager = ContentDetector = open_video = None
...
def _guard_transform(a, b, src, dst):
    ...
    warn = (
        f"transformação excessiva (ganho={[round(float(v), 3) for v in diag]}, "
        f"offset={[round(float(v), 3) for v in off]}) "
        f"-> usando apenas ganho {[round(float(v), 3) for v in gain]}"
    )
    return (
        torch.diag(gain),
...
def _correct_exposure(images, seams, edit_points, mode, method, strength,
    ...
    if mode == "anchor":
        ...
        if ref is None:
            print("[H3 Emendas 2.0/Bruxos] Pixels insuficientes no trecho de referência; exposição não alterada.")
            return logs
...
def _detect_shot_cuts(images, threshold, device, min_gap=_CUT_MIN_GAP):
    ...
    if not _SCENEDETECT_AVAILABLE:
        print(
            "[H3 Emendas 2.0/Bruxos] PySceneDetect não está instalado; "
            "usando a detecção interna por mudança de nível."
        )
        return None
    n_frames = images.shape[0]
    if n_frames < 2:
    ...
    try:
        ...
        if not writer.isOpened():
            raise IOError("Não foi possível criar o vídeo temporário para detectar cortes.")
        ...
        if tmp_path and os.path.exists(tmp_path):
            for _ in range(3):  
                ...
                print(f"[H3 Emendas 2.0/Bruxos] Aviso: não foi possível remover o temporário {tmp_path}.")

        print(f"[H3 Emendas 2.0/Bruxos] Cortes detectados: {cuts}")
        return cuts

    except Exception as e:
        print(f"[H3 Emendas 2.0/Bruxos] Falha no PySceneDetect: {e}")
        # 清理资源
        if video is not None:
            try:
                if hasattr(video, 'close'):
...
# ---------- 预设与工具提示 ----------
_PHOTO_TOOLTIP = (
    "Correção de cor/exposição: off desliga; low é conservador; medium é o ponto "
    "inicial; high usa transferência MKL com janela maior; max normaliza o nível do vídeo inteiro."
)

_SEAM_TOOLTIP = (
    "Correção de movimento por fluxo óptico e mistura local. Comece desligado; "
    "níveis mais altos corrigem mais frames, mas podem suavizar a imagem."
)

_FLASH_TOOLTIP = (
    "Corrige flashes acidentais de 1 ou 2 frames. Desligue em cenas com relâmpagos, explosões ou luzes rápidas intencionais."
)

_CUT_DETECT_TOOLTIP = (
    "Detecta cortes reais e impede que a correção atravesse de um plano para outro."
)
...
# ---------- ComfyUI 节点 ----------
class H3SeamCorrectionV2(io.ComfyNode):

    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="BruxosH3AutoContextSeamCorrectionV2",
            display_name="H3 Auto Chunks · Corrigir Emenda 2.0 (Bruxos)",
            category="Bruxos do VFX/MiniMax H3/Auto Chunks",
            description=(
                "Corrige emendas depois do VAE Decode. Usa o registro exato dos chunks "
                "quando a entrada info está ligada, alinha exposição/cor por trecho e, "
                "opcionalmente, corrige movimento com fluxo óptico sem atravessar cortes reais."
            ),
            inputs=[
                io.Image.Input(
                    "images",
                    tooltip="Vídeo completo já decodificado pelo VAE.",
                ),
                io.Dict.Input(
                    "info",
                    optional=True,
                    tooltip="Saída info do H3 Auto Chunks. Fornece as posições exatas das emendas e evita depender somente da detecção automática.",
                ),
                io.Combo.Input(
                    "fix_color_preset",
                ...
                io.Float.Input(
                    ...
                    max=1.0,
                    step=0.01,
                    tooltip="Área mínima do quadro que precisa apresentar o flash. Valores menores são mais agressivos; comece entre 0,20 e 0,40.",
                ),
                ...
                io.Float.Input(
                    ...
                    max=50.0,
                    step=0.5,
                    tooltip="Sensibilidade da detecção de cortes. Valores menores detectam mais cortes; comece entre 10 e 20.",
                ),
                io.Int.Input(
                    "blend_frames",
                    default=2,
                    ...
                    step=1,
                    tooltip=(
                        "Frames usados para suavizar a transição de nível na emenda. "
                        "Valores altos ficam mais graduais, mas podem borrar movimento rápido."
                    ),
                ),
                io.Boolean.Input(
                    "use_gpu",
                    default=True,
                    tooltip="Usa CUDA para estatística, transformação de cor e fluxo óptico.",
                ),
                io.Boolean.Input(
                    "cut_detection",
                    default=True,
    ...
    def execute(
        ...
        if device.type != "cuda":
            print(
                ...
                "CUDA indisponível; estatística e fluxo óptico usarão o dispositivo de entrada."
            )
        ...
        if isinstance(info, dict):
            expected_frames = int(info.get("decoded_frames", 0) or 0)
            candidates = info.get("boundaries") or []
            if not expected_frames or abs(expected_frames - n_frames) <= 2:
                exact_boundaries = sorted({
                    int(value) for value in candidates
                    if 2 < int(value) < n_frames - 2
                })
            elif candidates:
                print(
                    f"[H3 Emendas 2.0/Bruxos] O info pertence a {expected_frames} "
                    f"frames, mas o vídeo possui {n_frames}; usando detecção automática."
                )

        if exact_boundaries:
            print(
                f"[H3 Emendas 2.0/Bruxos] Emendas recebidas do Auto Chunks: "
                f"{exact_boundaries}."
            )
            detected_cuts = (
                _detect_shot_cuts(images, cut_threshold, device)
                if cut_detection else []
            )
            detected_cuts = detected_cuts or []
            edit_points = sorted(set(exact_boundaries + detected_cuts))
            seams = []
            for boundary in exact_boundaries:
                correlation = _spatial_similarity(
                    images,
                    left_start=max(0, boundary - 3), left_end=boundary,
                    right_start=boundary,
                    right_end=min(n_frames, boundary + 3), device=device,
                )
                _boundary_kind, boundary_delta = _classify_boundary(
                    images, boundary, _LOCAL_THRESHOLD, device
                )
                same_scene = correlation > 0.60 or boundary_delta < 0.08
                if not cut_detection or same_scene:
                    seams.append(boundary)
                    print(
                        f"[H3 Emendas 2.0/Bruxos] Frame {boundary}: mesma cena "
                        f"(similaridade={correlation:.3f}, diferença={boundary_delta:.3f}); "
                        "correção autorizada."
                    )
                else:
                    print(
                        f"[H3 Emendas 2.0/Bruxos] Frame {boundary}: corte real "
                        f"(similaridade={correlation:.3f}, diferença={boundary_delta:.3f}); "
                        "correção ignorada."
                    )
        elif cut_detection:
            print("[H3 Emendas 2.0/Bruxos] Detectando cortes com PySceneDetect.")
            cuts = _detect_shot_cuts(images, cut_threshold, device)
        
            if cuts is None:
                print("[H3 Emendas 2.0/Bruxos] PySceneDetect falhou; usando detecção por mudança de nível.")
                stats = _frame_stats(images, device)
                ...
                if not seams:
                    print("[H3 Emendas 2.0/Bruxos] Nenhuma emenda encontrada; vídeo preservado.")
                    return io.NodeOutput(images)

            else:
                if not cuts:
                    print("[H3 Emendas 2.0/Bruxos] Nenhum corte detectado.")
                    seams = []
                    edit_points = []
                else:
                    seams = []
                    ...
                    for c in cuts:
                        ...
                        if corr > 0.60:
                            seams.append(c)
                            print(f"[H3 Emendas 2.0/Bruxos] Frame {c}: mesma cena (similaridade={corr:.3f}); corrigindo cor.")
                        else:
                            print(f"[H3 Emendas 2.0/Bruxos] Frame {c}: corte real (similaridade={corr:.3f}); ignorado.")
                    if not seams:
                        print("[H3 Emendas 2.0/Bruxos] Todas as transições detectadas são cortes reais.")
        else:
            stats = _frame_stats(images, device)
            steps, z = _detect_step_boundaries(stats, _DETECT_WINDOW, _DETECT_Z, min_gap)
            steps = [s + 1 for s in steps if s + 1 < n_frames]
            ...
            edit_points = sorted(boundaries)
            if not seams:
                print("[H3 Emendas 2.0/Bruxos] Nenhuma emenda detectada.")

        if photo is None and seam is None and not fix_flash:
            print(
                "[H3 Emendas 2.0/Bruxos] Todas as correções estão desligadas; vídeo preservado."
            )
            return io.NodeOutput(images)
        ...
        if photo is not None and photo["mode"] == "flatten":
            ...
            print(
                f"[H3 Emendas 2.0/Bruxos] color[max]: normalização global; nível de referência="
                f"({float(ref[0]):.4f},{float(ref[1]):.4f},{float(ref[2]):.4f})"
            )
            print(
                f"[H3 Emendas 2.0/Bruxos] color[max]: variação de luminosidade "
                f"{float(before.max() - before.min()):.4f} -> "
                f"{float(after.max() - after.min()):.4f}  "
                f"(faixa de ganho {float(gain.min()):.2f}~{float(gain.max()):.2f})"
            )
            if clipped:
                print(
                    f"[H3 Emendas 2.0/Bruxos] Aviso: {clipped}/{n_frames} frames "
                    f"exigiam ganho acima do limite seguro {_GAIN_MAX}x. A informação "
                    "perdida em frames quase pretos não pode ser recuperada; gere novamente "
                    "o trecho, aumente context_frames ou use uma referência de exposição."
                )
            ...
            if not logs:
                print("[H3 Emendas 2.0/Bruxos] Alinhamento de cor/exposição não encontrou ajuste necessário.")
            for bnd, s, e, a, b, warn in logs:
                diag, luma_out = _describe_transform(a, b)
                after = _boundary_step(out, bnd, stat_window, device)
                print(
                    f"[H3 Emendas 2.0/Bruxos] Cor no frame {bnd}, trecho [{s},{e}): "
                    f"gain=({diag[0]:.3f},{diag[1]:.3f},{diag[2]:.3f}) "
                    f"degrau de luminosidade {before.get(int(bnd), 0.0):+.4f} -> {after:+.4f}"
                )
                if warn:
                    print(f"[H3 Emendas 2.0/Bruxos] Frame {bnd}, proteção de segurança: {warn}")
            ...
            if transients_after:
                ...
                for f in transients_after:
                    if 0 < f < out.shape[0] - 1:
                        ...
                        print(f"[H3 Emendas 2.0/Bruxos] Flash corrigido no frame {f}; área afetada {abnormal_ratio:.2%}.")
            else:
                print("[H3 Emendas 2.0/Bruxos] Nenhum flash residual detectado após a correção de cor.")
        ...
        if seam is not None or fix_flash:
            ...
            for b, kind, action in actions:
                print(f"[H3 Emendas 2.0/Bruxos] Frame {b}: {kind} -> {action}")
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "BruxosH3AutoContextSeamCorrectionV2":
        "H3 Auto Chunks · Corrigir Emenda 2.0 (Bruxos)",
}
